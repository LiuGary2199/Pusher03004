...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<MudHourJaw>()[0].LastSum != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
        ...
        if (Resources.FindObjectsOfTypeAll<ADWrapper>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("admanager key");
        }
        if (Resources.FindObjectsOfTypeAll<ADWrapper>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADWrapper>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<MudHourJaw>()[0].UtahTone != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADWrapper>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADWrapper>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADWrapper>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
        ...
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<CosmosTireWrapper>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<CosmosTireWrapper>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<CosmosTireWrapper>();
            }
        }
        GameObject.Find("MainManager").GetComponent<CosmosTireWrapper>().VoyageID = info.Adjust_APP_ID;

        if (!GameObject.Find("MainManager").GetComponent<DivaAnWrapper>())
        {
            GameObject.Find("MainManager").AddComponent<DivaAnWrapper>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<DivaAnWrapper>().appid = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<DivaAnWrapper>().appid = info.PackageName;
#endif
        ...
        Resources.FindObjectsOfTypeAll<MudHourJaw>()[0].UtahTone = info.GameCode;
        Resources.FindObjectsOfTypeAll<MudHourJaw>()[0].LastSum = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<MudHourJaw>()[0].LastGrantSum = info.BaseUrl + CScream.GrantSum;
        Resources.FindObjectsOfTypeAll<MudHourJaw>()[0].LastScreamSum = info.BaseUrl + CScream.ScreamSum;
        Resources.FindObjectsOfTypeAll<MudHourJaw>()[0].LastFastSum = info.BaseUrl + CScream.FastSum;
        Resources.FindObjectsOfTypeAll<MudHourJaw>()[0].LastCosmosSum = info.BaseUrl + CScream.CosmosSum;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CScream.ScreamSum + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CScream.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CScream.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif

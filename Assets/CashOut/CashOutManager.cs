...
public class CashOutManager : MonoWeightily<CashOutManager>
{
    [Header("登录平台")]
    ...
    private void Start()
    {
        Account = ToilHallWrapper.YewCarpet("CashOut_Account");
        Money = ToilHallWrapper.YewBylaw("CashOut_Money");
    }
    ...
    private void OnApplicationPause(bool pauseStatus)
    {
        ...
        if (pauseStatus && Seconds > 0)
        {
            ...
            string info = $"All {MudHourJaw.instance.SuchWay_Hall.MoneyName} have been converted,Please check your rewards!";
            PhotographerWrapper.Vocation.RomanPhotographer();
            PhotographerWrapper.Vocation.CorridorPhotographer(title, info, (int)Seconds);
            for (int i = 0; i < 10; i++) // 10次延时 10800秒 3小时
                PhotographerWrapper.Vocation.CorridorPhotographer(title, info, (int)Seconds + (i * 10800));
        }
    ...
    void TimeCount() //计时
    {
        if (Data != null)
        {
            ...
            //任务
            if (MudHourJaw.instance.SuchWay_Hall != null && MudHourJaw.instance.SuchWay_Hall.TaskList.Count > 0)
            {
                //记录第一次登录日期utc
                ...
                if (Day >= MudHourJaw.instance.SuchWay_Hall.TaskList.Count) //一天一任务 天数超出任务数量显示默认任务
                    Data.TaskData = MudHourJaw.instance.SuchWay_Hall.TaskList.FirstOrDefault(t => t.IsDefault);
                else
                    Data.TaskData = MudHourJaw.instance.SuchWay_Hall.TaskList[Day];
    ...
    {
        Money += Value;
        ToilHallWrapper.HubBylaw("CashOut_Money", Money);
    ...
    public void SendEvent1304() ////打点 关闭商店后行为
    {
        ...
        if (Event_1304Time <= 0)
            return;
        DaleBulgeScript.YewVocation().PoolBulge("1304", Event_1304Time.ToString());
        Event_1304Time = 0;
    }
    ...
    Dictionary<string, string> Headers() // 请求头
    {
        ...
        return new Dictionary<string, string>
        {
            ...
            {"Authorization", ToilHallWrapper.YewCarpet("CashOut_Token")},
    ...
    public void Login() // 登录
    {
        ...
        {
            Platform = "Android";
            DeviceAdId = ToilHallWrapper.YewCarpet("gaid");
            if (Application.platform == RuntimePlatform.Android)
            {
        ...
        else
        {
            Platform = "iOS";
            Manufacturer = "Apple";
            DeviceAdId = ToilHallWrapper.YewCarpet("idfv");
        }
        ...
        bool hasuuidAndAppid = ToilHallWrapper.YewShop("UuidAndAPPid");
        ...
        MudFailWrapper.YewVocation().KillDaleGain(
            url: loginUrl,
            jsonData: jsonBody,
            success: (result) =>
            {
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        bool isNewPlayer = !PlayerPrefs.HasKey(CScream.If_HeRubObtain + "Bool") || ToilHallWrapper.YewShop(CScream.If_HeRubObtain);
                        if (isNewPlayer)
                            ToilHallWrapper.HubShop("UuidAndAPPid", true);
#endif

                        //刷新token 获取提现规则
                        ToilHallWrapper.HubCarpet("CashOut_Token", response.data.token);
                        GetWithdrawRule();
                        //整理数据
                        ...
                        if (PlayerPrefs.HasKey("CashOut_ConvertTime"))
                            Data.ConvertTime = long.Parse(ToilHallWrapper.YewCarpet("CashOut_ConvertTime"));
                        ...
                        ToilHallWrapper.HubCarpet("CashOut_ConvertTime", Data.ConvertTime.ToString());
                    ...
                    else
                    {
                        ...
                        CedarWrapper.YewVocation().SinkCedar("Login fail :" + response.msg);
                    }
                }
                catch (Exception e)
                {
            ...
            fail: () =>
            {
                ...
                CedarWrapper.YewVocation().SinkCedar("Login fail");
                Ready = false;
            },
            timeout: 3f,
            headers: Headers()
    ...
    public void UpdateUserInfo() // 更新用户信息
    {
        ...
        MudFailWrapper.YewVocation().KillYew(
            url: url,
            success: (result) =>
            {
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        double OldCash = ToilHallWrapper.YewIndigo("CashOut_Cash");
                        ...
                        if (Money == 0)
                        {
                            ...
                            //打点 如果钱转化了 上报转化信息
                            if (IsIconFly)
                                DaleBulgeScript.YewVocation().PoolBulge("1302", Event_Money, NewCash.ToString());
                        }
                        else
                        {
                            Data.Cash = NewCash;
                        ...
                        ToilHallWrapper.HubIndigo("CashOut_Cash", Data.Cash);
    ...
    public void Withdraw() // 提现
    {
        ...
        {
            CedarWrapper.YewVocation().SinkCedar($"Minimum withdrawal amount {MinWithdrawCount}");
        ...
        MudFailWrapper.YewVocation().KillDaleGain(
            ...
            success: (result) =>
            {
                ...
                catch (Exception e)
                {
                    ...
                    _CashOutPanel?.CloseLoading_Withdraw();
                    CedarWrapper.YewVocation().SinkCedar("Withdraw fail :" + e.Message);
    ...
    {
        DaleBulgeScript.YewVocation().PoolBulge("1303", Event_Cash, IsSuccess ? "1" : "0");
    }
    ...
    public void ReportAdjustID() // 上报adjust_id
    {
        ...
        requestData.id = CosmosTireWrapper.Instance.YewCosmosEdge();
        string jsonBody = JsonMapper.ToJson(requestData);
    ...
    {
        if (string.IsNullOrEmpty(ToilHallWrapper.YewCarpet("CashOut_Token")))
        {
    ...
    public void AddTaskValue(string Name, float Value) //增加任务值
    {
        if (Data.TaskData != null && Data.TaskData.Name == Name)
        {
            ...
            {
                DaleBulgeScript.YewVocation().PoolBulge("1305", Name, NewValue.ToString(), Data.TaskData.IsDefault.ToString());

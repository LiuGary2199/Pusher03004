...
public class CashOutPanel : LastUITruck
{
    ...
    private void Start()
    {
        CashOutManager.YewVocation()._CashOutPanel = this;
        CloseBtn.onClick.AddListener(() => { PianoUIArid(nameof(CashOutPanel)); });
        CashOutBtn.onClick.AddListener(() => { OnCashOutBtn(); });
        MoneyName = MudHourJaw.instance.SuchWay_Hall.MoneyName;
        InfoText.text = MudHourJaw.instance.SuchWay_Hall.Description;
        InfoPanel_OpenBtn.onClick.AddListener(() => OpenPanel(InfoPanel));
        InfoPanel_CloseBtn.onClick.AddListener(() => ClosePanel(InfoPanel));
        ContactBtn.GetComponent<Text>().text = MudHourJaw.instance.LastSum;
        ContactBtn.onClick.AddListener(() =>
        {
            string url = MudHourJaw.instance.LastSum;
        ...
        RecordPanel_OpenBtn.onClick.AddListener(() =>
        {
            ...
            CashOutManager.YewVocation().GetWithdrawRecord();
        });
        ...
        AccountText.text = CashOutManager.YewVocation().Account;
        Confirm_AccountText.text = CashOutManager.YewVocation().Account;
        if (string.IsNullOrEmpty(AccountText.text))
        {
            AccountText.text = "Account";
            Confirm_AccountText.text = "Account";
        }
        IDText.text = "UID:" + CashOutManager.YewVocation().Data.UserID;
        PolicyBtn.onClick.AddListener(() =>
        {
            string url = MudHourJaw.instance.LastSum + "/privacy_policy.html";
#if UNITY_ANDROID || UNITY_EDITOR
            Application.OpenURL(url);
#elif UNITY_IOS
        openUrl(url);
        ...
        PolicyToggle.isOn = ToilHallWrapper.YewShop("CashOut_PolicyAgree");
    ...
    {
        if (CashOutManager.YewVocation().Data == null)
            return;
        if (string.IsNullOrEmpty(CashOutManager.YewVocation().Account))
        {
            OpenPanel(AccountPanel);
            return;
        }
    ...
    void OnConfimBtn() //确认修改账户
    {
        ...
        {
            CedarWrapper.YewVocation().SinkCedar("Account cannot be empty");
            return;
        }
        if (!string.IsNullOrEmpty(AccountErrorText.text))
        {
            CedarWrapper.YewVocation().SinkCedar(AccountErrorText.text);
            return;
        }

        CashOutManager.YewVocation().Account = AccountInput.text;
        ToilHallWrapper.HubCarpet("CashOut_Account", AccountInput.text);
    ...
    void OnSbmitBtn() //确认是否同意协议 同意后调用提现接口
    {
        ...
        {
            CedarWrapper.YewVocation().SinkCedar("Please agree to the policy");
            return;
        }
        ToilHallWrapper.HubShop("CashOut_PolicyAgree", true);
        StartLoadingAnim(CashOutBtn_Loading);
        StartLoadingAnim(SbmitBtn_Loading);
        CashOutManager.YewVocation().Withdraw();
    }
    ...
    public void UpdateMoney()
    {
        ...
        float MoneyEnd = CashOutManager.YewVocation().Money;
        DOTween.To(() => MoneyStart, x => MoneyText.text = x.ToString("F2"), MoneyEnd, 1f);
        CashText.text = CashOutManager.YewVocation().Data.Cash.ToString("F2");
        //进度条动画
        float MaxMoney = float.Parse(MudHourJaw.instance.SuchWay_Hall.convert_goal, CultureInfo.CurrentCulture);
    ...
    public void MoneyToCashAnim()
    {
        ...
        float CashOutEnd = CashOutManager.YewVocation().Data.Cash;
    ...
    {
        if (CashOutManager.YewVocation().Data.TaskData != null && CashOutManager.YewVocation().Data.TaskData.Name != "Null")
        {
            // Cash余额任务 不走前端进度统计 使用后台Data.Cash数值
            if (CashOutManager.YewVocation().Data.TaskData.Name == "Cash" && CashOutManager.YewVocation().Data.Cash < CashOutManager.YewVocation().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.YewVocation().Data.TaskData.Description, CashOutManager.YewVocation().Data.Cash, CashOutManager.YewVocation().Data.TaskData.Target);
            }
            // 其他任务 前端统计任务完成进度 
            else if (CashOutManager.YewVocation().Data.TaskData.Name != "Cash" && CashOutManager.YewVocation().Data.TaskData.NowValue < CashOutManager.YewVocation().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.YewVocation().Data.TaskData.Description, CashOutManager.YewVocation().Data.TaskData.NowValue, CashOutManager.YewVocation().Data.TaskData.Target);
            }
            // 任务完成 显示提现按钮
            else
            {
    ...
    public void UpdateRecord(int Page = 0) //更新单页10个提现记录
    {
        ...
        if (CashOutManager.YewVocation().Data.Record == null || CashOutManager.YewVocation().Data.Record.Count == 0)
        {
            NoRecordTip.SetActive(true);
            return;
        }

        int RecordCount = CashOutManager.YewVocation().Data.Record.Count;
        ...
        List<WithdrawRecordItem> OnePageRecord = CashOutManager.YewVocation().Data.Record.GetRange(startIndex, itemsToShow);
    ...
    public void CloseLoading_Withdraw(bool IsCashOutSuccess = false)
    {
        ...
        if (IsCashOutSuccess)
        {
            CedarWrapper.YewVocation().SinkCedar("Withdraw success");
            ClosePanel(ConfirmAccountPanel);
        }
    }
    ...
    {
        DaleBulgeScript.YewVocation().PoolBulge("1301", LeftTimeText.text, CashOutManager.YewVocation().Money.ToString());
    }
}

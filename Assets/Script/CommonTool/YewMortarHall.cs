...
{
    public static void Aloud<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class YewMortarHall : MonoWeightily<YewMortarHall>
{
    // Start is called before the first frame update
    public bool SiteGrower;
    public bool WeCoil;

    static public YewMortarHall instance;
    ...
    /// <returns></returns>
    public Vector3 EraSpiteHairy(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject YewCreeperUtahOutlet(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float HairyMeSmite(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Smite180Me360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int YewSinceHarmfully(Vector2 p1, Vector2 p2)
    {
        float angle = Smite180Me360(HairyMeSmite(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 EraDistortFeudBurn(Renderer Renderer)
    {
    ...
    public Vector2 EraDistortFeudBurn(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 EraHumbleBurn(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 EraUrbanSeverity()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = EraUpwindLight();
        float screenH = EraUpwindParent();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int EraFrenchPeart(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] YewTankerWherever(int total, int count)
    {
    ...
    public Dictionary<string, object> CarpetMeMeteoritic(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string MeteoriticMeCarpet(Dictionary<string, object> Info)
    {
    ...
    public int DayPorkDiff(string dateStart, DateTime dateEnd)
    {
    ...
    public int SecPorkRite(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan PorkRite(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 YewEonOrCent(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = YewEonOrCent(canvas, text, strFragmentIndex);
            Vector3 lastPos = YewEonOrCent(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = YewEonOrCent(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 YewEonOrCent(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float MultiplyByExam(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Spot= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string KingdomDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static  string UnaidedDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Spot;

...
/// 负责处理跨平台的通知功能
/// </summary>
public class PhotographerWrapper : MonoBehaviour
{
    public static PhotographerWrapper Vocation{ get; private set; }
    
    // 通知渠道ID
    private const string CHANNEL_ID= "game_notifications";
    
    // 可配置的URL Scheme，用于iOS唤起应用
    private string OftBoomer= "yourgame://";
    ...
    private void Awake()
    {
        ...
        StartCoroutine(RetreatPhotographerPercussive());
    }

    IEnumerator RetreatPhotographerPercussive()
    {
        // 仅在移动平台请求通知权限
    ...
    /// 初始化通知系统
    /// </summary>
    private void PowerfullyComplementary()
    {
        RomanPhotographer();
        StatuePhotographerLegally();
    }

    public void RomanPhotographer()
    {
        #if UNITY_ANDROID
    ...
    /// 创建Android通知渠道
    /// </summary>
    private void StatuePhotographerLegally()
    {
        #if UNITY_ANDROID
    ...
    /// 发送即时通知
    /// </summary>
    public void PoolSuccessorPhotographer(string title, string message)
    {
        #if UNITY_ANDROID || UNITY_IOS
        #if UNITY_ANDROID
    ...
    /// 发送定时通知
    /// </summary>
    public void CorridorPhotographer(string title, string message, int secondsFromNow)
    {
        #if UNITY_ANDROID || UNITY_IOS
        #if UNITY_ANDROID
        var notification = new AndroidNotification
    ...
    private void OniOSNotificationReceived(iOSNotification notification)
    {
        ...
        if (lastNotification != null)
        {
            ...
            // 确保应用在前台运行
            if (!Application.isFocused && !string.IsNullOrEmpty(OftBoomer))
            {
                Application.OpenURL(OftBoomer);
                Debug.Log($"通过URL scheme({OftBoomer})唤起游戏");
            }
        }
    }
    #endif
